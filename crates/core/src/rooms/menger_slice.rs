...
/// True if the unit-square point survives Menger-face recursion of depth d.
...
/// Menger sponge face-slice room.
#[derive(Debug, Default)]
pub struct MengerSlice {
    seed: u64,
}
...
impl Room for MengerSlice {
    ...
    fn motif(&self) -> Option<crate::motifs::Motif> {
        Some(crate::motifs::Motif {
            ...
            encodes: "center squares vanishing at every scale",
        })
    }
    ...
    fn status(&self, t: f64) -> Option<String> {
        let d = depth(t, None);
        let cells = 8usize.pow(d as u32);
        Some(format!("depth={d}  cells~{cells}  DRAG:DEPTH"))
    }
    ...
    fn status_input(&self, t: f64, inputs: &[RoomInput]) -> Option<String> {
        ...
        let d = depth(t, hands.last().copied());
        let frac = (8.0f64 / 9.0).powi(d as i32);
        Some(format!("DEPTH={d}  area~{frac:.3}"))
    }

    fn reveal(&self) -> &'static str {
        "The Menger sponge removes the open middle cross of each cube, forever. \
         A face of the construction is this square carpet of holes: zero area in \
         the limit, infinite surface, topological dimension one."
    }
}

#[cfg(test)]
mod tests {
    use super::{MengerSlice, filled};
    use crate::canvas::Canvas;
    use crate::room::{Room, RoomInput};

    #[test]
    fn status_invites() {
        let s = MengerSlice::new().status(0.3).unwrap();
    ...
    #[test]
    fn center_opens() {
        assert!(!filled(0.5, 0.5, 1));
        assert!(filled(0.1, 0.1, 1));
    }

    #[test]
    fn render_ink() {
        let mut c = Canvas::new(40, 28);
        MengerSlice::new().render(&mut c, 0.5);
        assert!(c.ink_count() > 20);
    }

    #[test]
    fn motif_ok() {
        assert!(MengerSlice::new().motif().unwrap().line.len() >= 6);
    }
}
