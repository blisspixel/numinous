//! Smale horseshoe (toy): stretch, fold, and return a square strip.
//!
//! Discrete geometry of the horseshoe map. DRAG: SET THE ITERATIONS.
//! See `docs/ROOMS.md`.

use crate::room::{MAX_ROOM_POKES, Room, RoomInput, RoomMeta};
use crate::surface::Surface;

fn phase_unit(t: f64) -> f64 {
    if t.is_finite() {
        t.clamp(0.0, 1.0)
    } else {
        0.0
    }
}

fn finite_pokes(pokes: &[(f64, f64)]) -> Vec<(f64, f64)> {
    let start = pokes.len().saturating_sub(MAX_ROOM_POKES);
    pokes[start..]
        .iter()
        .copied()
        .filter(|&(x, y)| x.is_finite() && y.is_finite())
        .map(|(x, y)| (x.clamp(0.0, 1.0), y.clamp(0.0, 1.0)))
        .collect()
}

fn iters(t: f64, hand: Option<(f64, f64)>) -> usize {
    if let Some((x, _)) = hand {
        (1 + (x * 5.0) as usize).clamp(1, 6)
    } else {
        (1 + (phase_unit(t) * 4.0) as usize).clamp(1, 5)
    }
}

/// One horseshoe step on unit square: vertical stretch, fold into two strips.
fn horseshoe(x: f64, y: f64) -> Option<(f64, f64)> {
    // Map vertical strips: bottom half stretches to full, top half folds.
    if y < 0.5 {
        Some((x * 0.4 + 0.1, y * 2.0))
    } else {
        // Fold: reverse x, place in right strip.
        Some((0.9 - x * 0.4, 2.0 * (1.0 - y)))
    }
}

fn draw(canvas: &mut dyn Surface, n: usize, seed: u64) {
    let (width, height) = canvas.draw_bounds();
    if width == 0 || height == 0 {
        return;
    }
    let grid = 56usize;
    let shift = if seed == 0 {
        0.0
    } else {
        (seed % 9) as f64 * 0.01
    };
    for iy in 0..grid {
        for ix in 0..grid {
            let mut x = (ix as f64 + 0.5) / grid as f64;
            let mut y = (iy as f64 + 0.5) / grid as f64;
            x = (x + shift).fract();
            let mut ok = true;
            for _ in 0..n {
                if let Some((nx, ny)) = horseshoe(x, y) {
                    x = nx;
                    y = ny;
                } else {
                    ok = false;
                    break;
                }
            }
            if !ok {
                continue;
            }
            let px = (x.clamp(0.0, 1.0) * width.saturating_sub(1) as f64).round() as i32;
            let py = ((1.0 - y.clamp(0.0, 1.0)) * height.saturating_sub(1) as f64).round() as i32;
            let ch = if n > 3 {
                '#'
            } else if n > 1 {
                '*'
            } else {
                '+'
            };
            canvas.plot(px, py, ch);
        }
    }
    // Square outline.
    let x1 = width.saturating_sub(1) as i32;
    let y1 = height.saturating_sub(1) as i32;
    canvas.line(0, 0, x1, 0, '.');
    canvas.line(0, y1, x1, y1, '.');
    canvas.line(0, 0, 0, y1, '.');
    canvas.line(x1, 0, x1, y1, '.');
}

/// Smale horseshoe toy room.
#[derive(Debug, Default)]
pub struct Horseshoe {
    seed: u64,
}

impl Horseshoe {
    /// Create the room with default seed (0).
    #[must_use]
    pub fn new() -> Self {
        Self { seed: 0 }
    }
    /// Create with variation seed.
    #[must_use]
    pub fn new_with(seed: u64) -> Self {
        Self { seed }
    }
}

impl Room for Horseshoe {
    fn meta(&self) -> RoomMeta {
        RoomMeta {
            id: "horseshoe",
            title: "Smale Horseshoe",
            wing: "Motion & Dynamics",
            blurb: "Stretch and fold a square into a horseshoe: chaos geometry. t and DRAG: SET \
                    THE ITERATIONS.",
            accent: [200, 100, 40],
        }
    }

    fn render(&self, canvas: &mut dyn Surface, t: f64) {
        draw(canvas, iters(t, None), self.seed);
    }

    fn postcard_t(&self) -> f64 {
        0.65
    }

    fn motif(&self) -> Option<crate::motifs::Motif> {
        Some(crate::motifs::Motif {
            key: "horseshoe",
            root: 392.0,
            tempo: 132,
            line: &[0, 1, 7, 8, 12, 13, 7, 0],
            encodes: "stretch fold return: symbol dynamics on a square",
        })
    }

    fn verb(&self) -> Option<&'static str> {
        Some("DRAG: SET THE ITERATIONS")
    }

    fn status(&self, t: f64) -> Option<String> {
        let n = iters(t, None);
        Some(format!("iters={n}  strips~{}  DRAG", 1usize << n.min(8)))
    }

    fn render_poked(&self, canvas: &mut dyn Surface, t: f64, pokes: &[(f64, f64)]) {
        let hands = finite_pokes(pokes);
        let n = iters(t, hands.last().copied());
        draw(canvas, n, self.seed ^ hands.len() as u64);
        if let Some(&(x, y)) = hands.last() {
            let (width, height) = canvas.draw_bounds();
            if width > 0 && height > 0 {
                let px = (x * width.saturating_sub(1) as f64).round() as i32;
                let py = (y * height.saturating_sub(1) as f64).round() as i32;
                canvas.line(px - 2, py, px + 2, py, 'o');
                canvas.line(px, py - 2, px, py + 2, 'o');
            }
        }
    }

    fn status_input(&self, t: f64, inputs: &[RoomInput]) -> Option<String> {
        let pokes = crate::pokes_from_inputs(inputs);
        let hands = finite_pokes(&pokes);
        if hands.is_empty() {
            return self.status(t);
        }
        let n = iters(t, hands.last().copied());
        Some(format!("ITERS={n}  strips~{}", 1usize << n.min(8)))
    }

    fn reveal(&self) -> &'static str {
        "Smale's horseshoe is the geometric heart of chaos: stretch a square, \
         fold it, and map it back. The invariant set is conjugate to a shift on \
         two symbols, and nearby maps inherit the same symbolic dynamics."
    }
}

#[cfg(test)]
mod tests {
    use super::Horseshoe;
    use crate::canvas::Canvas;
    use crate::room::{Room, RoomInput};

    #[test]
    fn status_invites() {
        let s = Horseshoe::new().status(0.3).unwrap();
        assert!(s.contains("DRAG") || s.contains("iters"));
        assert!(s.chars().count() <= 56);
    }

    #[test]
    fn iters_change() {
        let r = Horseshoe::new();
        let o = r.status(0.2).unwrap();
        let a = r
            .status_input(
                0.2,
                &[RoomInput::PointerDown {
                    x: 0.95,
                    y: 0.5,
                    t: 0.0,
                }],
            )
            .unwrap();
        assert_ne!(o, a);
    }

    #[test]
    fn render_ink() {
        let mut c = Canvas::new(40, 28);
        Horseshoe::new().render(&mut c, 0.5);
        assert!(c.ink_count() > 20);
    }

    #[test]
    fn motif_ok() {
        assert!(Horseshoe::new().motif().unwrap().line.len() >= 6);
    }
}
