...
fn param_a(_t: f64, hand: Option<(f64, f64)>, seed: u64) -> f64 {
    ...
    if let Some((x, _)) = hand {
        ...
        // Ambient a holds a readable pear; motion lives in the draw.
        1.1 + s
    }
}
...
fn draw(canvas: &mut dyn Surface, a: f64, show: f64, seed: u64) {
    ...
    // Soft ghost of the full pear (upper and lower).
    ...
    // Bright pear so far, stem to body.
    let mut tip = (ox.round() as i32, cy.round() as i32);
    for sign in [1.0_f64, -1.0] {
        let mut prev: Option<(i32, i32)> = None;
        for i in 0..=drawn {
            let u = i as f64 / steps as f64;
...
mod tests {
    ...
    fn ambient_pen_moves_the_plate() {
        ...
        r.render(&mut a, 0.15);
        r.render(&mut b, 0.75);
        assert_ne!(a.to_text(), b.to_text(), "pen must draw the pear");
