//! Svensson attractor: another classic trigonometric map.
//!
...
fn params(t: f64, hand: Option<(f64, f64)>, seed: u64) -> (f64, f64, f64, f64) {
    ...
    if let Some((x, y)) = hand {
        (1.0 + x * 1.5 + s, 1.0 + y * 1.5, 1.5, 1.5)
    } else {
        let u = phase_unit(t);
        (1.4 + u * 0.3 + s, 1.56, 1.40, -6.56 + u * 0.2)
    }
}
...
impl Room for Svensson {
    ...
    fn motif(&self) -> Option<crate::motifs::Motif> {
        Some(crate::motifs::Motif {
            ...
            root: 246.94,
            tempo: 88,
            line: &[0, 3, 6, 10, 13, 10, 6, 3],
            encodes: "scaled sin of x minus sin of y",
        })
    }
    ...
    fn reveal(&self) -> &'static str {
        "Johnny Svensson's map is a trigonometric cousin of Clifford and de Jong: \
         four parameters, dense orbits, and attractors that look hand-drawn by chaos."
    }
}
...
mod tests {
    ...
    #[test]
    fn status_invites() {
        let s = Svensson::new().status(0.3).unwrap();
    ...
    fn render_ink() {
        ...
        Svensson::new().render(&mut c, 0.5);
        assert!(c.ink_count() > 20);
    }

    #[test]
    fn motif_ok() {
        assert!(Svensson::new().motif().unwrap().line.len() >= 6);
    }
}
