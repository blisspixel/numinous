...
fn draw(canvas: &mut dyn Surface, a: f64, seed: u64) {
    ...
    // parametric: x = a cos t, y = a sin t cos t
    let steps = 360;
    let mut prev: Option<(i32, i32)> = None;
    for i in 0..=steps {
        let th = rot + std::f64::consts::TAU * (i as f64 / steps as f64);
        let x = rad * th.cos();
        let y = rad * th.sin() * th.cos();
...
/// Lemniscate of Gerono room.
#[derive(Debug, Default)]
pub struct LemniscateGerono {
    seed: u64,
}

impl LemniscateGerono {
...
impl Room for LemniscateGerono {
    ...
    fn motif(&self) -> Option<crate::motifs::Motif> {
        Some(crate::motifs::Motif {
            ...
            encodes: "algebraic eight distinct from Bernoulli infinity",
        })
    }
    ...
    fn reveal(&self) -> &'static str {
        "The lemniscate of Gerono is the figure-eight x = a cos t, y = a sin t \
         cos t. Unlike Bernoulli's infinity sign it is a rational quartic, a \
         clean algebraic eight."
    }
}

#[cfg(test)]
mod tests {
    use super::LemniscateGerono;
    use crate::canvas::Canvas;
    use crate::room::{Room, RoomInput};

    #[test]
    fn status_invites() {
        let s = LemniscateGerono::new().status(0.3).unwrap();
        assert!(s.contains("DRAG") || s.contains("gerono"));
        assert!(s.chars().count() <= 56);
    }

    #[test]
    fn scale_changes() {
        let r = LemniscateGerono::new();
    ...
    fn postcard_has_ink() {
        ...
        LemniscateGerono::new().render(&mut c, 0.5);
