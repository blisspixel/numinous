...
**Pre-production (heading toward 0.1).** These docs are the blueprint. Nothing is built yet. The next step is **0.1 Foundations** (the stack, the headless core, all three faces in skeleton, the test loops), then **0.2 The Vertical Slice** (one flagship room, fully polished, to prove the feel). The version-gated plan, with 1.0 and 2.0+ defined by quality bars rather than dates, is in [`docs/ROADMAP.md`](docs/ROADMAP.md).
