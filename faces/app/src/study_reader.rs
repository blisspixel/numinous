...
mod tests {
    ...
    fn unavailable_mathematics_and_languages_are_explicit_without_locking_notes() {
        // A room with no authored treatment, so the reader has to say so. Golden
        // Angle used to be that example and now has one of its own.
        let mut reader = reader("cellular-automata", "haw");
