...
fn server_instructions() -> &'static str {
    concat!(
        "Begin at the three doors from list_rooms: touch one astonishing room, walk the ordered Strange Loop chain, or wander by wing. ",
        "Call watch_show when you want the house to direct that six-room chain one bounded cue at a time. The caller owns timing: follow its explicit next call by choice, and use motion reduced for one postcard look. It stores no cursor or progress, reads no journal or workspace, and never returns the explanation. ",
        "The complete compatible catalog remains in structuredContent.rooms. describe_room is a safe doorway and never returns the explanation. Its optional journalCue says only that this local player profile kept exact room evidence; it never opens the text or changes the workspace. Follow its explicit workspace retrieval call only by choice. ",
    ...
    if let Some(tools) = catalog.get_mut("tools").and_then(Value::as_array_mut) {
        for tool in tools {
            for key in ["inputSchema", "outputSchema"] {
                if let Some(schema) = tool.get_mut(key).and_then(Value::as_object_mut) {
...
fn sliders_schema() -> Value {
    json!({
        "type": "array",
        "maxItems": numinous_core::MAX_NAMED_SLIDERS,
        "description": "Named extra knobs besides a. Each item is name=value, name=value:min:max, or an object with name and optional value, min, and max. Omitted range is -2 to 2. Omitted value is 1. Names the formula does not use are refused.",
        "items": {
            "oneOf": [
                { "type": "string", "minLength": 1 },
                {
                    ...
                    "required": ["name"],
                    "properties": {
                        "name": { "type": "string", "minLength": 1, "maxLength": numinous_core::MAX_SLIDER_NAME_CHARS },
                        "value": { "type": "number" },
                        "min": { "type": "number" },
                        "max": { "type": "number" }
                    },
...
fn build_tools_catalog() -> Value {
    json!({
        "tools": [
            {
                "name": "list_rooms",
                "description": "Open the room threshold. structuredContent.threshold offers exactly three doors: touch the flagship, walk the ordered Strange Loop chain, or wander by wing. structuredContent.chain carries the six nonspoiling walk steps, wings carries compact wing doorways, and rooms retains the complete compatible catalog. The existing starters field is retained. For a shorter rendering of the same typed result, pass response_mode compact.",
            ...
            },
            {
                ...
                "description": "Create in Formula Jam / Studio. Draw one graph with expr, one parametric path with x_expr and y_expr over t, or one field over the plane when expr uses y, z, i, re, im, arg, or conj, or when ymin, ymax, or reading is set. Height and phase fields sing their reading along the real axis; the zero reading is a proof and stays silent. Curated recipe and seeded discovery remain graph paths. Optional auto_step with seed walks the same bank like Auto without session state. Pass list_recipes true to inspect the graph bank. Pass list_experiments true for bundled Studio capsules: each row names next as open_creation with the experiment id already bound, so a packaged player can open them without a host file. family returning-home, shape-and-scale, three-readings, named-sliders, overlay, euclidean, two-voices, or notes selects one set. Unary functions: sin cos tan exp ln abs sqrt floor, and for fields re im arg conj. Pair functions: mod min max euclid. Tracker pattern: pat(x..x..x.) or the editor form x..x..x. Named pitches: note(\"c e g\"), with . a rest. Constants: pi, e, and for fields i. An integer 0/1 graph also names pattern as tracker text and grid as a numbered step grid. The sung MIDI voice also names roll as a piano-roll grid, pitch over time. A successful plot names next as save_creation with the expression and window already bound. Recipe lists, experiment lists, and errors do not.",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "expr": {
                        ...
                        "family": {
                            "type": "string",
                            "enum": ["returning-home", "shape-and-scale", "three-readings", "named-sliders", "overlay", "euclidean", "two-voices", "notes"],
