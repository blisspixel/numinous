...
- Lorenz gets a divergence readout, the chaos emblem made tactile (the Storm and Hawking both singled it out in the alien playtest). A live `TWINS n APART AT RHO r` status integrates two starts a breath apart at the current rho and measures how far they end up: as the sweep raises rho from the edge of chaos into deep chaos, sensitive dependence pulls the pair steadily further across the attractor and the number climbs (about 11 to 36), a live measure of how unforecastable the system has become. Because it moves, Lorenz now poses predictions and challenges too, the seventh room to do so, and predicting a chaotic spread is exactly the hard, honest guess the predict keystone is for.
