...
one graph or one complete `x(t), y(t)` pair and a named pitch map. The third
growth ring, `NUMINOUS_STUDIO 4`, carries optional prose credit, a capped
printable-ASCII sentence the forker can edit. The fourth growth ring,
`NUMINOUS_STUDIO 5`, carries a field over the plane: `kind=field`, a stored
...
`pat(x..x..x.)` in that same language. Pattern Studio's piano roll remains
a later ring. This is the Baba Is You and
