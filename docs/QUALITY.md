...
- **Enforced now:** formatting, Clippy and rustdoc with warnings denied,
  doctests, 3,551 passing all-target test cases plus three expensive ignored
  diagnostics, locked
  builds, house style, `cargo-deny` in CI, an 80% line-coverage floor, and a
  three-OS test-and-build matrix. The current measured coverage is 95.32%
  regions and 95.34% lines under the documented exclusions. Each of the four
...
- **Catalog plate quality (cycles 161 to 168, machine path):** full-catalog
  ...
  right-hand self-similar). Soft-thin large plates (density under 0.02 or under
  280 ink on 160x90) are tracked and densified with floors, double strokes, and
  filled lobes. Art-first interaction: no pointer reticle or drag trail painted
  over the room math. **Live motion:** ambient phase should change the raster
  for curve, wave, and process rooms (pen travel, construction roll, scroll,
  unfurl, breathe); targeted tests assert distinct frames at different t.
  Cycles 164 to 167 cover rolling cardioids and cycloids, pens on classical
  polar and algebraic curves, unfurling spirals, and breathing or scrolling
  waves. Cycle 168 re-zeroed Log Spiral phase-thin and four dead-domain rooms.
  These scans are engineering evidence, not stranger plate-beauty
  certification. Fun and beauty still need hallway observation.
  ...
  the shared quantizer's projection of the App source; MCP tests compare every event for all 354 rooms, enforce a
...
- **Not yet evidenced:** a completed stranger hallway test, accessibility review
  ...
  telemetry, platform code signing and notarization, runtime-resolved native
  dependency versions, embedded per-binary Rust reachability, and independent
...
1. **First-contact and accessibility group:** draw several unlike app profiles,
   ...
   first action.
2. **Interaction and game-flow group:** traverse all 354 rooms through immediate
