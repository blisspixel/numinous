...
- **Current interaction:** The ordinary App visit opens at K=2 and waits for a
  ...
  into four lobes. In the optional App experiment, observing it offers an
  earned connection. [Study](STUDY.md) is independently available before any
  play. Resolution-aware chord sampling keeps compact CLI output legible.
  App, CLI, and MCP share the parameter and accepted-hand model; the legacy
  CLI/MCP reveal paths retain their own progression rules.
