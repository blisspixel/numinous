...
- **Done:** the headless core (`Room` trait with `reveal()`, deterministic ASCII `Canvas`, seeded RNG, registry, `verb`, `render_poked`, and variation); the CLI face (`numinous`), the MCP face (`numinous-mcp`), and the windowed app; **30 catalog rooms across 10 wings** plus hidden content; 6 lever-driven sims; 11+ games; the full engineering harness (edition-2024 workspace, pinned toolchain, `-D warnings`, cargo-deny, house-style guard, an 80% line coverage gate, three-OS CI). Current local evidence: fmt, clippy, 881 tests, locked build, Windows verify, 90.49% region cover, and 90.08% line cover all green.
...
(3) six first pokes, now generalized into 28 interactive rooms with verbs;
...
never come. 28 of 30 catalog rooms now answer the hand; the-pour and
slope-rider remain.
