...
- **Tracked follow-ups (from the July 2026 bug-hunts and the two playtest rounds, see `docs/PLAYTESTS.md`):** the Lorenz divergence readout reworked to be born from a tiny perturbation and climb monotonically (the Storm; today it reads a two-point distance that starts high and wanders); predict extended to let a mind predict the *rate* and see the *shape* of its error, not a bare score (Sage and Data converged); a reactive room whose motion answers being watched, a predator-prey pulse for the instinct-only mind (the Xenomorph); the cross-room identity bridges the reveals do not yet name (Ramanujan: the logistic map is the Mandelbrot set along its real axis; the cardioid shared by Times Tables, Mandelbrot, and Epicycles); and the remaining low-severity persistence durability items surfaced by the round-2 audit (a genuinely atomic Windows replace to close a transient missing-file read window, a parent-directory fsync, and cleanup of orphaned temp/lock files on error paths), none of which corrupt data given the delta-merge model; and the CPU render-performance cliffs a round-3 audit measured at maximized-window sizes (the Mandelbrot and Julia CPU fallback at seconds per frame, Voronoi's per-pixel sqrt scan, and Arecibo's 24 full-surface repaints per drag), most of which a single live-render resolution cap below 4096 (render smaller, blit up, keeping exports full-resolution) would retire at once, a sharpness-versus-framerate design choice worth making deliberately. Resolved since these were first listed: the Cairn reciprocity whisper, the L-System growing upward, the daily-seed midnight race, the daily-streak regression, and fast crash-lock recovery are all built (`CHANGELOG.md`).
