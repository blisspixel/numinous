...
- **Done (the window arcade):** Munch and Nim play inside the app alongside the quiz, cursor-driven and keyboard-native, on the daily seeds, posting to the shared table and leveling the shared journey; Mobius and Zeno's Square join the catalog.
- **Next (in order):** Engine B (the ElevenLabs radio); the music visualizer (system loopback + FFT); the Gauntlet in the window; GPU paths for more rooms; gamepad support; hexaflexagons and the Hat tile; per-era chiptune voicing.
