...
- **Done:** the headless core (`Room` trait with `reveal()`, deterministic ASCII `Canvas`, seeded RNG, registry, `verb`, `render_poked`, and variation); the CLI face (`numinous`), the MCP face (`numinous-mcp`), and the windowed app; **31 catalog rooms across 10 wings** plus hidden content; 6 lever-driven sims; 11+ games; the full engineering harness (edition-2024 workspace, pinned toolchain, `-D warnings`, cargo-deny, house-style guard, an 80% line coverage gate, three-OS CI). Current local evidence: fmt, Clippy, 1,191 all-target test cases, locked build, Windows release gate, 93.17% region coverage, and 92.82% line coverage all pass.
...
- **Done (controller exploration and games):** `gilrs` 0.11.2 provides
  ...
  pause menu, and focus transitions drain queued hardware events. Deadzone,
...
creation and persistence, and 1,191 passing all-target test cases. Required public CI is
...
| Rigor and care are provable | 1,191 all-target test cases, 92.82% measured line coverage, Clippy, style, and supply-chain CI | Independent math review, MSRV, accessibility, real-hardware soak, and artifact provenance |
...
| Beautiful and honest throughout | An exact 240-screen matrix and a 42-lens review cover every catalog room plus the captured game, overlay, Show, Studio, reset, and phase branches | Perceptual regression, representative human judgment, uncaptured persistent states, and removal of every unsupported claim |
...
- **Beauty QA:** a deterministic 240-screen matrix covers six states per room
  plus every persistent game display branch, overlays, The Show, Studio, and
  reset and phase flows. It enforces inventory, dimensions, nonblank frames,
