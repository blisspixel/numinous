...
**Current release state: 0.4.0-alpha.15, Understanding Alpha active with its exit
...
| **6. Creator depth on the built loop** | **The named scalar grammar rung is built in alpha.15:** `floor`, Euclidean `mod`, `min`, and `max` run through one bounded core parser and evaluator, four new curated recipes, App help, CLI and MCP discovery, portable capsule round trips, every face, and a real stdio session. Next rungs: the parametric pair as the multi-expression capsule ring with scale quantization beside it, then MIDI and audio exports, then editable prose credit. **A second variable remains the large unlock**, because it turns a language of curves into a language of fields and opens domain coloring, implicit curves and height maps for one grammar change | The creator ladder keeps rising without waiting on later community publishing, and each rung ships with its own machine gates; freestyle play is bounded by vocabulary long before it is bounded by rendering |
...
cohort passes all 27 comparisons, including all four functions in the current
grammar rung. `ROOM_CATALOG` now owns every listed room's
...
4. **The instrument rungs sharpen (rock 6 in the table).** The scalar grammar
   ...
   through every face. The parametric pair is the already-planned
   multi-expression ring and goes next; scale quantization lands with it
...
- **Done:** the headless core (`Room` trait with `reveal()`, deterministic ASCII `Canvas`, seeded RNG, registry, `verb`, `render_poked`, and variation); the CLI face (`numinous`), the MCP face (`numinous-mcp`), and the windowed app; **355 catalog rooms** plus hidden content; 6 lever-driven sims; 11+ games; the full engineering harness (edition-2024 workspace, pinned toolchain, `-D warnings`, cargo-deny, house-style guard, an 80% line coverage gate, three-OS CI). Current local evidence: fmt, Clippy, 3,790 passing all-target test cases plus three expensive ignored diagnostics, locked build, Windows release gate, 94.25% region coverage, and 94.27% line coverage all pass.
...
The package is **0.4.0-alpha.15**. The 0.1 Public Foundation exit criterion is
...
Package label is **0.4.0-alpha.15** because Understanding Alpha is the active
