...
| 0.6 Portable | Partial | release packaging, engagement smoke judged on signal and PNG geometry rather than file size and run against a freshly built binary rather than whichever one was on disk, provenance/SBOM, install/play/uninstall roundtrip with all three player-owned files preserved, not just the journey, run nightly against a freshly packaged archive as well as on the tagged artifact |
