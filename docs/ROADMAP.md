...
- **Built:** a plotted or sung Studio experiment names `next` as
  ...
  App cells. Tracker editing is built: type `x..x..x.` or `pat(x..x..x.)`.
  Piano roll and the rest of the in-app capability quest remain later rungs.
...
| Real creative surface | Studio graphs, paired parametric paths, fields over the plane with three stored readings, named sliders with declared ranges, overlay programs of two to four graphs, and Euclidean rhythms as `euclid(hits, steps)`, tracker marks as `pat(x..x..x.)` or the editor form `x..x..x.`, continuous and named-scale voices, versioned `.num` serialization with title, author, era, lineage, and editable prose credit, links, plotting, animation, singing, WAV and Standard MIDI File export of the sung melody on CLI, MCP, and the App F4 share, exact paused App reopen, the one-key share bundle, the local Gallery wall, and fork with recorded descent exist | Pattern Studio piano roll, MusicXML, safe share preview for incoming links, and clean-install round trip |
