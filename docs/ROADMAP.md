...
- **Next (in order):** more games in the window (munch, nim); Engine B (the ElevenLabs radio); the music visualizer (system loopback + FFT); GPU paths for more rooms; gamepad support; more Shape & Space rooms (Mobius, hexaflexagons, the Hat tile); per-era chiptune voicing.
