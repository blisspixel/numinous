...
- **Done:** the headless core (`Room` trait with `reveal()`, deterministic ASCII `Canvas`, seeded RNG, registry, `verb`, `render_poked`, and variation); the CLI face (`numinous`), the MCP face (`numinous-mcp`), and the windowed app; **31 catalog rooms across 10 wings** plus hidden content; 6 lever-driven sims; 11+ games; the full engineering harness (edition-2024 workspace, pinned toolchain, `-D warnings`, cargo-deny, house-style guard, an 80% line coverage gate, three-OS CI). Current local evidence: fmt, Clippy, 1,230 all-target test cases, locked build, Windows release gate, 93.34% region coverage, and 93.10% line coverage all pass.
...
- **Done (Engine A2 listening refinement):** the App no longer doubles motifs
  ...
  catalog motif expands into a deterministic 32-step stereo arrangement with
  rests, four-bar phrase development, a final root resolution, and quiet root
  and fifth anchors. Source changes crossfade. Smoothed master and focus gain
  preserve the playhead, including radio; device-rate tests cover 44.1, 48, 96,
  and 192 kHz. Structural audio checks cover seams, bounds, headroom, DC, and
...
- **Done (Galton causal experiment loop):** the completed pile no longer moves
  ...
  participant evidence remain open. The next deeper loop is Game of Life.
...
creation and persistence, and 1,230 passing all-target test cases. Required public CI is
...
| Rigor and care are provable | 1,230 all-target test cases, 93.10% measured line coverage, Clippy, style, and supply-chain CI | Independent math review, MSRV, accessibility, real-hardware soak, and artifact provenance |
...
| Beautiful and honest throughout | An exact 253-screen matrix and a 42-lens review cover every catalog room plus the captured game, input-aware controller, pause, overlay, Show, Studio, reset, and phase branches | Perceptual regression, representative human judgment, uncaptured persistent states, and removal of every unsupported claim |
...
- **Beauty QA:** a deterministic 253-screen matrix covers six states per room
  plus every persistent game display branch, overlays, The Show, Studio, and
  reset and phase flows, with 13 compact controller and pause receipts. It
  enforces inventory, dimensions, nonblank frames,
  deterministic opening states, and at least 100 changed raw room-content
  pixels against a same-phase baseline, plus coarse support, adjacent-tile, and
  color-change floors,
