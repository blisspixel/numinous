...
- **In progress (the founder's directive, July 2026): The Next Wave of rooms.** Twenty-nine new room designs across four aspects (physics, deep mathematics, fun-first, cosmic), recorded in `ROOMS.md` under "The Next Wave", ranked by wow-to-build. **First eight built** plus wave depth: Sandpile, Chladni, Ripple, Coffee Cup, Ford Circles, Zeta Walk, Starbow, Slingshot, and The First Rain (`first-rain`, site percolation spanning cliff near p_c=0.5927). Machine work continues without waiting on human magic proof; each room still faces the full Definition of Done, and every non-textbook reveal claim carries a source.
