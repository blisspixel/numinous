...
7. **The structural debts, scheduled rather than deplored.** Cross-face
   ...
   adapter, reducing that coordinator again to 5,048 lines. Existing helper
   modules retain device normalization, hit testing, and gesture mechanics;
   core retains domain rules and persistence primitives. The 159 CLI command,
   persistence, rendering, game, and terminal regressions now live in a sibling
   test module with private access preserved, leaving the production coordinator
   near 5,000 lines. Stateless MCP replay
