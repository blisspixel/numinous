...
| 0.7 Creator | Partial | CLI save/reopen `.num` gate, including that a saved range and knob come back and change the drawing rather than only being echoed; CLI and MCP plot parity locked across 12 cases; App curve framing pinned to the same core rule, and now the columns each face draws in as well, since framing alone let an off-by-one in the App's sample grid through; the singularity case that exists to prove both sides discard the same point ran only at even widths, where the grid straddles it, so it had never discarded anything, and odd widths now run that path with a guard that fails if they drift back; MCP cannot open a `.num` at all; App gallery open |
