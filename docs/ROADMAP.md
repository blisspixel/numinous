...
- **In progress (the founder's directive, July 2026): The Next Wave of rooms.** Twenty-nine new room designs across four aspects (physics, deep mathematics, fun-first, cosmic), recorded in `ROOMS.md` under "The Next Wave", ranked by wow-to-build. **Built:** The Sandpile (`sandpile`); Chladni Figures (`chladni`, free-plate nodal sand, DRAG: TUNE THE PLATE, drive tone is the modes, Gordon-Webb-Wolpert reveal). Still designed, not built: Ripple Tank, Coffee Cup, Ford Circles, Zeta Walk, Starbow, Slingshot, and the rest of the wave. Machine work continues without waiting on human magic proof; each room still faces the full Definition of Done, and every non-textbook reveal claim carries a source.
