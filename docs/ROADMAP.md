...
An evidence-labeled plan from empty repo to a living world. Each milestone has
a **goal**, concrete **deliverables**, an explicit **exit criterion** (how we
know it is done), and the **risk it retires**.
...
**Current release state: 0.4.0-alpha.25, Understanding Alpha active with its exit
...
- **Built:** a plotted or sung Studio experiment names `next` as
  ...
  math. It is not a lobby, inventory, or loadout gate. Named sliders are
  built: extra identifiers besides `a` bind a value and a declared range,
  version 6 capsules, and family `named-sliders`. Overlay programs are
  built: two to four graphs share one window as `NUMINOUS_STUDIO 7`, family
  `overlay`. Overlay WAV mixes every graph; MIDI stays the first curve.
  Euclidean rhythms are built: `euclid(hits, steps)` places
  ...
  (`closing-voices`, `wandering-voices`). The first Pattern Studio view
  is built: pattern text of integer 0/1 graphs (`x..x..x.` for tresillo).
  Tracker editing, step grid, piano roll, and the rest of the in-app
  capability quest remain later rungs.
...
| **6. Creator depth on the built loop** | **The first multi-expression ring is built in alpha.16:** one atomic `x(t), y(t)` pair draws a planar path and sings its y coordinate through continuous, chromatic, major, minor, or pentatonic pitch mapping. Version 3 capsules carry the pair and scale through the App, CLI, MCP, Gallery, exact reopen, and lineage-preserving fork while old version 1 and 2 documents stay unchanged. **MIDI export of that sung melody is built on all three faces:** core writes a Standard MIDI File type 0 from the same SoundSpec (nearest 12-TET key, leftover cents as pitch bend over plus or minus two semitones, range declared as Registered Parameter Number 0). CLI `numinous sing 'sin(t)' --out song.mid`, MCP `sing_expression` `midi: true`, and the App F4 share as `melody.mid` share it. WAV was already the audio export. **Editable prose credit is built:** a fork offers `After {title} by {author}` from the parent, the forker can rewrite or clear it, and version 4 capsules carry that sentence through every face. **The MCP keep chain is named in the packaged player docs:** a plot or song returns `next` at `save_creation` with the expression and window already bound; save, open, and fork return `next` at `fork_creation` with the capsule already bound as parent. `PLAY.md` and the play-numinous skill say so. Those doors answer a successful glance or keep; they are not a lobby or a loadout gate. **The second independent variable is built:** a field over the plane with phase, height, and proved-zero readings, version 5 capsules, and family `three-readings`. Fields are seen first; they have no melody yet. **Named sliders are built:** extra identifiers besides `a` bind a value and a declared range, version 6 capsules, CLI `--slider`, MCP `sliders`, App Tab selection, and family `named-sliders`. **Overlay programs are built:** two to four graphs share one window, version 7 capsules, editor form `sin(x) & cos(x)`, family `overlay`. Every overlay graph sings in WAV; MIDI stays the first curve. **Euclidean rhythms are built:** `euclid(hits, steps)` places onsets as evenly as possible, family `euclidean`. **Pattern text is built:** integer 0/1 graphs report tracker marks (`x..x..x.` for tresillo). Next rungs: editable tracker, step grid, piano roll, and the in-app capability quest | The creator ladder keeps rising without waiting on later community publishing, and each rung ships with its own machine gates; freestyle play is bounded by vocabulary long before it is bounded by rendering |
...
5. **Numinous Encounter Receipts, built:** a versioned native receipt binds
   replay ABI, compatibility fingerprint, normalized action, result digest, and
   provenance. `play_room` emits it only when `receipt` is true. It is a
   Numinous replay and provenance artifact, not an Open Knowledge Format
   Attested Computation Receipt or OKF projection. Asking does not keep the
   play.
6. **Explicit promotion, built:** the player chooses whether a Numinous
   Encounter Receipt becomes a journal subject by passing it to
   `record_journal`. The server replays the action and keeps only a live
   match. No Numinous Encounter Receipt, render, action, result,
...
9. **MCP creation parity, built:** `save_creation`, `open_creation`, and
   `fork_creation` return canonical `.num` text, a native link, exact preview,
   and a journal-ready subject without reading or creating host files. Title,
   author, era, and parent lineage use the same core capsule
   semantics as the other faces. CLI and MCP share one core fork operation.
   The calls remain private under Watch Agent policy, and journal v3 can retain
   every valid capsule link exactly while migrating strict v2 input.
10. **Remembered-room doorway, built:** `describe_room` reports only that this
    local player profile has current exact-subject journal evidence for the
    room. It does not return journal contents or mutate the workspace. The cue
...
13. **Show for minds, built:** `watch_show` carries the six-room Strange Loop
    score one complete cue per call. Core owns route order, nonspoiling
    questions, exact phases, and deterministic variation. MCP returns one or
    three ASCII looks, visual alternatives, typed deltas and invariants, sound
    facts, and optional audio. Continuation, restart, and leave are explicit
    calls. No timer, cursor, task, progress write, journal read, workspace read,
    reveal, concept, or citation is hidden inside the experience.
14. **Reflection proposals:** only after retrieval, correction, export, and
...
1. **GPU post feasibility, built and measured:** the disabled-by-default
   ...
   the AMD Radeon 780M Vulkan reference, five-pass p95 was 2.558 ms at 1080p
   and 4.431 ms at 1440p; the wider upload-through-readback boundary was 9.961
   ms and 13.709 ms. The exact revision, binary digest, raw samples, budgets,
   and limits live in `docs/evidence/sensory-post-spike-2026-08-25.json`.
2. **CPU comparison, measured and rejected:** an equivalent reusable
   single-threaded `f32` path measured 38.151 ms p95 at 1080p and 83.955 ms at
   1440p against 33 ms and 50 ms budgets. This rejects that implementation,
   not every possible CPU design. Optional PNG previews and a focused
   neighbor-lighting test keep the comparison tied to visible bloom.
3. **Direct presentation, built and measured:** one shared pipeline now targets
   either the deterministic offscreen validator or an acquired sRGB surface.
   The surface path negotiates a compatible adapter and format, requests FIFO
   pacing with one frame in flight, and tone maps directly into the acquired
   texture without an offscreen output copy or readback. On the same Windows
   AMD Radeon 780M Vulkan reference, combined acquire-through-present-request
   p95 was 17.447 ms at 1080p and 18.378 ms at 1440p, with zero transient or
   suboptimal frames across 120 retained samples at each size. The raw receipt,
   exact revision, binary digest, window contract, and limits live in
...
4. **Real App integration and portable platform proof, built; physical pacing
   ...
   A separate verifier recomputes each receipt's raw summaries and segment
...
7. **The structural debts, scheduled rather than deplored.** Cross-face
   ...
   disjoint. One typed core Gauntlet now owns all four seeded stages, stage and
   ...
   protocol adapter. Seeded prediction and touch or parameter challenge posing,
   grading, structured result projection, and progress accounting live in a
   focused challenge adapter, while deterministic goal construction and grading
   remain in core and routing stays in the entry point. MCP journal reads,
   writes, corrections, exports, erasure,
   ...
   of importing it backward from the entry point. CLI accessibility switch
   interpretation, `NO_COLOR` policy, known-limit disclosure, and fixed-width
   terminal reporting now live in one face-local module, while the command
   entry point only gathers raw environment values and prints the report.
   Bounded CLI render dimensions, static hand points, pointer gestures, their
   mutual exclusion, and interaction-aware status projection now live in one
   face-local adapter reused by render, share, sonify, and Studio callers. The
   corresponding MCP adapter owns bounded JSON hand points and gestures, their
   ...
   focused face-local adapter too. MCP Formula Jam discovery, portable creation
   projection, optional audio attachment, and encounter receipts now live in a
   corresponding focused Studio-tool adapter. Core retains Studio evaluation,
   rendering, request bounds, melody construction, capsule validity, and
   lineage semantics. Shared bounded CLI game records,
   overflow resynchronization, neutral departure presentation, and the `?`
   explanation door now live in a focused face-local adapter too. Game rules,
   progression, scoring, and concepts remain outside it. CLI game session
   presentation, input loops, Journey events, and score posting for all
   standalone games plus the fixed five-run Bench now live in a focused
   runtime adapter. Core retains seeded rules, deals, legality, and grading,
   while command validation and the outer Journey persistence envelope remain
   in the root coordinator. The 124 App lifecycle,
   input, audio, presentation, Studio, and game regressions now live in a
   sibling test module with private access preserved, leaving the production
   entry point at 6,521 lines. App Quiz, Munch, Nim, Arcade, and Gauntlet
   session entry, keyboard handling, score posting, and transient feedback
   audio now live in a focused game runtime adapter, leaving the production
   entry coordinator at 5,948 lines. App global audio keys, pointer gestures,
   wheel updates, and controller commands now live in a focused input runtime
   adapter, reducing that coordinator again to 5,048 lines. App room exports,
   Studio naming and sharing, Gallery navigation and reopen transitions, and
   bounded creation opening now live in a focused creation runtime adapter,
   reducing that coordinator again to 4,491 lines. App source selection, gain
   and mute handling, radio timing, Studio and Watch Agent playback handoff,
   room voices, transient sounds, and visualizer fallback now live in a focused
   audio runtime adapter, reducing that coordinator again to 3,997 lines.
   ...
   focused room runtime adapter, reducing that coordinator again to 3,208
   lines. The adapter uses core's canonical engineered-room membership and
   keeps elapsed-time guards deterministic; core still owns every mathematical
   state machine, grade, and truth.
   Existing helper modules retain rendering, discovery, validation,
   serialization, device playback, sound synthesis, device normalization, hit
   testing, gesture mechanics, and domain rules. The 163
   CLI command, persistence, rendering, game, and terminal regressions now live
   in a sibling test module with private access preserved, leaving the
   production coordinator at 3,848 lines. Stateless MCP replay
   and result projection for Hackenbush, Party, Fifteen, Quiz, Munch, Munch
   Arcade, and Nim, together with the shared score table projection, now live
   in a focused face-local game-tool adapter. Core retains deals, legality,
   ...
   in a focused protocol adapter. Request progress mapping, daily seed freezing,
   local-store resolution, score and Journey persistence, and response-visible
   save failures now live in a focused progress adapter. Keyless argument
   parsing, earned-state projection, overlays, and bounded consolidation for the
   seven engineered flagship Aha arcs now live in a focused adapter while core
   retains their state machines and truth. Room discovery, description, gated
   reveal projection, structured sound, bounded play rendering, temporal
   evidence, and encounter receipts now live in a focused room adapter while
   core retains the registry, veil rules, rendering, sound generation, goals,
   grading, and mathematical truth. Crack, SETI, Aliens, and Gauntlet request
   parsing and structured presentation now live in a focused puzzle adapter
   while core retains seeded generation, rules, legality, grading, and truth.
   Cairn reading and leaving, boon choice, the trophy case, and the Journey
   dashboard now live in a focused journey adapter while core retains bequest
   encoding, factor reading, unlock, boon, trophy, progression, scoreboard, and
   persistence truth. Connection-scoped viewer lifecycle, one-use pairing
   guidance, consent status projection, and private-activity-safe result
   presentation now live in a focused broadcast adapter. The broadcast
   transport and consent state machine retain session, queue, framing, and
   compatibility truth. Exhaustive public, private, and control policy,
   public-call capture, daily replay normalization, journey-blind result
   projection, and event commit now live in a focused viewer projection
   adapter, leaving the production entry point near 500 lines.
   Bounded runtime validation of the declared
   JSON Schema subset, guiding argument-name suggestions, and the temporal and
   Arcade cross-field checks now live in a focused schema adapter while the
   catalog remains the immutable protocol contract. The remaining god-file
   seams continue under the drag report's map alongside the Sensory Lift.
...
Workstream 7 already schedules the debts. The first four extractions are complete:
...
cohort passes all 34 comparisons, including three parametric paths and four
named pitch maps. `ROOM_CATALOG` now owns every listed room's
...
354-constructor hand list, and metadata-only lookups do not allocate rooms. A
typed `GauntletPuzzle` now owns the seeded run, grades, combo total, canonical
reveals, and score identity for every face and the Watch Agent verifier. The
core local-state resolver owns environment precedence and all managed default
paths, while complete erasure includes the opt-in journal under the same lock,
preflight, and residue contract as the other stores. These four extractions
...
1. **Time and company over MCP.** Exact two-observation evidence with a typed
   ...
   parity, the three-door threshold, the portable typed evidence capsule, and
   the caller-paced Show for minds are built. Next comes cross-platform Sensory
   Lift proof before the later authored arc.
...
proves connection works. The Studio's ceiling is one parser change away. A
digital mind can now make, reopen, and fork a titled, signed,
lineage-carrying object over MCP without granting the server host filesystem
access.
...
1. **Truth debts in the mathematics, fixed first** (they are the honesty
   ...
   one capsule singing three different default songs across the faces is
   resolved at 32 notes through one core `SingRequest` contract.
...
3. **The edges pass rides with time-and-company (rock 2).** The fifteen
   ...
   making the myth and the math one object. MCP creation parity is built:
   save, open, and fork return portable capsules, and the exact link can be a
   player-chosen journal subject. The pride gap was a time-and-company gap.
4. **The instrument rungs sharpen (rock 6 in the table).** The scalar grammar
...
- **Done (agents as peers, v2):** 40 MCP tools total: 24 public play tools,
  fifteen private progression, creation, or local-state tools, and one local
  broadcast consent control. The surface has structured output and full CLI
  ...
  (transparency first, erasure on explicit consent, the welfare doctrine in
  `AGENT_PLAY.md`); `play_room` supports stateless per-call variation and
  normalized hand points; agents see, hear, create, play, level to 42, and post
  to the same score table; every play schema advertises an additive
  `response_mode`, with stable full tool-call results and nonexpanding compact
  text for complete structured result families; the player's manual speaks to
  humans, agents, and digital consciousnesses; the whole stdio face is proven
...
- **Done (local-model player lane, August 13, 2026):** one already-installed
  tool-capable Ollama model can enter through the real MCP server while an
  observer watches visible words and witnessed calls. Literal-loopback and
  local-weight checks forbid paid or remote fallback, the Numinous profile is
  disposable, transcript capture is opt-in, and private reasoning is excluded.
  CI tests the harness contract without running inference. A real first run
  exposed invented narration of calls that never executed, so the report now
  distinguishes claims from witnessed actions and permits one factual retry.
  This is exploratory synthetic evidence, not a consciousness or enjoyment
  result.
...
- **Done (Times Tables technical Flagship Proof):** the ordinary App visit holds
  ...
  remain later human-evidence work and do not hold back the current 0.3 line.
...
- **Done (CLI game runtime slice):** Bench and all eleven standalone terminal
  game sessions now live in `faces/cli/src/game_runtime.rs`; the command
  coordinator is 3,848 lines and retains validation plus final Journey
  persistence. The 163 sibling CLI regressions preserve private helper access,
  and focused cases lock neutral Bench departure, partial-round score keys,
  abandoned aggregate state, deterministic daily bookkeeping, and the derived
  command catalog. CLI all-target tests, Clippy with warnings denied, game
  truth, and color-free terminal contracts pass locally.
- **Done (app hardening slice):** app-local play state plus quiz deal/answer flow now live in `faces/app/src/play.rs`, pure game-screen rendering lives in `faces/app/src/game_draw.rs`, room chrome plus arrival-card hinting live in `faces/app/src/hud.rs`, help, journey, and banner overlays live in `faces/app/src/overlays.rs`, transient feedback banner construction and ticking live in `faces/app/src/feedback.rs`, shared in-window Munch grid, Nim heap/take, and Munch Arcade action controls live in `faces/app/src/controls.rs`, left-mouse mode decisions and pointer-state guards live in `faces/app/src/mouse_input.rs`, room navigation, re-deal, poke-history, drag-trail, and room-card tick helpers live in `faces/app/src/room_input.rs`, Studio text, parse, audio-spec, and curve drawing state live in `faces/app/src/studio_panel.rs`, explicit F9 hallway-test note capture lives in `faces/app/src/playtest.rs`, live-state PNG postcard export lives in `faces/app/src/postcard.rs`, bounded radio cache discovery, open-handle WAV validation, live-position math, and track loading live in `faces/app/src/radio_cache.rs`, game session coordination lives in `faces/app/src/game_runtime.rs`, mode-aware device coordination lives in `faces/app/src/input_runtime.rs`, export, Studio share, Gallery transition, and creation-open coordination lives in `faces/app/src/creation_runtime.rs`, source selection, radio timing, playback handoff, room voices, transient sounds, and visualizer fallback live in `faces/app/src/audio_runtime.rs`, and live room coordination lives in `faces/app/src/room_runtime.rs`. Room action copy is centralized in `numinous-core`: App arrival cards use touch-first fallback copy, while CLI live play and MCP room tools use neutral fallback copy. Tests cover shared game hit-test layout, raster output across quiz, Munch, Munch Arcade, Nim, every live Gauntlet stage, quiz daily seeding, no-repeat quiz history, answer acceptance, action-naming arrival cards, Studio chrome suppression, Studio panel editing and bounded drawing, cross-face action hints, shared Munch/Nim/arcade controls, room-input bounds, modal-safe pointer-state transitions, playtest-critical overlays, feedback banner copy/lifetimes, radio-volume banner retention, GPU/raster banner compositing, local playtest-note reports that align to the hallway-test prompts without collecting personal data, postcard PNGs that include pokes, the selected Visual Era, collision-safe filenames, bounded/sorted station cache discovery, low-sorted corrupt-track handling before the track cap, corrupt-track rejection, open-handle size rechecks, high-rate-device caps, non-wrapping live offsets, app radio recovery after a bad cached file, staged-wager membership through the core room set, all seven morph guard contexts, and persisted consolidation idempotence. The event-loop file remains a hotspot at 3,208 lines, but domain rules stay in `crates/core` and the refactor is moving in small verified modules.
- **Done (CI integrity baseline, August 27, 2026):** pull requests compare
  dependency changes and reject newly introduced moderate or higher known
  vulnerabilities. A pinned two-language CodeQL matrix analyzes Rust and
  ...
  domain-language and test-fixture findings carry narrow per-alert dispositions,
  while eight visual variation selectors now use accurate stream identifiers.
  Both queries and every analyzed source path remain enabled.
...
- **Done (hands-on room correction, July 13, 2026):** Galton now uses
  ...
  invariant tests and the regenerated 2,945-screen matrix cover these claims;
...
- **Done (0.3 Formula Jam discovery, machine path):** Studio F2 Random, F3 Auto
  ...
  Formative stranger sessions remain open on the later human-evidence track;
  they do not reopen the met 0.3 agent-and-machine exit.
- **Designed (Frontier and universal wonder wave, July 2026 research pass):** a
  ...
  carefully labeled frontier gestures). Full cards live in `ROOMS.md`. This is
  a catalog ambition ledger for Phase F and 1.x, not a completion claim for a
  later milestone.
...
- **Done (Kepler speed wager, fifth aha room):** one completed eccentricity
  tuning primes faster, slower, or same near the sun. App keys and its bottom
  band, plus MCP `speed_wager`, reach the same pure five-beat machine and bind
...
- **Done (source-blind packaged playtest correction, August 15, 2026):** an
  external agentic player exercised the published Linux CLI and MCP binaries
  without reading Rust source and supplied exact calls plus repeated
  reproductions. Safe description is now separate from earned reveal; the
  seven withheld wager beats retain the commitment but hide every grade and
  answer-bearing field; Times Tables goal state and status share one live K=5
  predicate; Kepler aliases share one canonical Journey star; Galton prime
  hides the theoretical mode; the terminal action teaches the working phase
  doorway; and local profile overrides name their file-path contract. Real CLI
  and MCP subprocess tests lock the reported paths. The same App polish slice
  makes Q quit from fullscreen and restores Skip Track as a full-size Settings
  action. The unregistered Understanding Alpha dry-run advances to protocol
  `0.4-v6`, whose fourth call earns the room reveal in one stateless request
...
- **Done (Mind's Seat session workspace, August 20, 2026):** `workspace` holds
  compact visit state in the current MCP process: place, intention, pending
...
- **Done (Mind's Seat remembered-room doorway, August 25, 2026):**
  `describe_room` can say that this local player profile kept exact current-room
  evidence without opening entry text or changing the process-local workspace.
  Its typed cue points to the explicit retrieval operation, disappears after
  erasure, and reports unreadable storage without exposing the journal path.
...
- **Done (Mind's Seat MCP creation parity, August 22, 2026):** three private
  ...
  link, exact preview, identity, era, lineage, and an explicit journal subject.
  CLI and MCP share the core fork operation. Journal v3 expands exact subjects
  to the bounded capsule capacity and migrates strict v2 journals. Core, schema,
  handler, and real stdio regressions prove identity bounds,
...
- **Done (Mind's Seat Show for minds, August 26, 2026):** `watch_show`
  presents the core-owned six-room Strange Loop score one bounded cue per
  call. Sampled motion returns arrival, postcard, and curtain looks; reduced
  motion returns the same cue's postcard. Every result includes exact ASCII,
  a visual alternative, typed deltas and held evidence, bounded sound inventory
  with explicit detail truncation, optional WAV audio, and explicit replay,
  next, restart, and leave
  calls. Runtime validates the closed output schema. The public Watch Agent
  projection omits audio bytes, remains more than 1 KiB inside its 64 KiB
  event bound, and fails closed on contract drift. Core, MCP, compact/full,
  no-progress, viewer, and real two-process stdio regressions cover all three
...
- **Done (Parrondo policy wager, sixth aha room):** one completed rule
  ...
  claim was mathematically false for its probabilities, so the canonical
  policy is now ABB. The truth and morph use an exact three-residue Markov
  expectation after 120 turns: A and B lose alone while ABB wins. Sampled
  walks remain tactile texture but never grade the call. Four selections earn
...
- **Done (manifest capsule first growth ring, and fork with lineage):** the
  ...
  rather than ignoring them; an unknown future header is refused by name. Links carry
...
- **Done (catalog growth through 354):** invent-and-ship past the early Next
  ...
  Breadth did not advance the version by itself; the completed 0.3 tactile bar
  does. See `CHANGELOG.md` and `ROOMS.md` Built now.
...
- **Done (flagship mathematical truth pass, cycle 133):** Formula Jam now gives
  ...
  in principle. Human evidence remains open for the 0.8 and 1.0 track.
- **Done (truthful complete local erasure, cycle 134):** CLI and MCP `forget`
  now inventory Journey, scores, player-owned local Cairn drafts, the opt-in
  experience journal, the generated radio cache, and the App crash diagnostic
  with resolved paths, bytes, counts, sidecar residue, and explicit exclusions.
  Preview remains non-destructive, Journey-only confirmation remains compatible,
  each other store has a separate consent flag, and `all_local` selects every
  managed store. Shared core
  ...
  deliberately outside this player-state command. Human evidence remains open
  for the 0.8 and 1.0 track.
- **Done (physics and geometry consequence depth, cycle 120):** Berry Phase,
  ...
  duplicate-history stability. Human evidence remains open for the 0.8 and 1.0
  track.
...
- **Done (0.4 Understanding Alpha prep):** added an opt-in, player-owned MCP
  experience journal. The `Journal` tracks timestamped room encounters,
  creations, and connections. It is fully integrated into persistence and
  backed by `read_journal`, `record_journal`, `correct_journal`,
  `export_journal`, and `erase_journal`. Native and Open Knowledge Format v0.2
  export are bounded and in-memory. The dedicated `erase_journal` tool remains
  available, while the later complete-erasure contract also exposes an
  explicit journal selection and includes it in confirmed `all_local` erasure.
...
- **Done (Cycle 21 first-contact and flagship goldens, August 2, 2026):**
  `agent-first-contact.py` cold-starts 40 tools, multi-wing play, munch, journal
...
six sims, three faces, 40 MCP tools, deterministic creation and persistence.
...
| Real creative surface | Studio graphs, paired parametric paths, fields over the plane with three stored readings, named sliders with declared ranges, overlay programs of two to four graphs, and Euclidean rhythms as `euclid(hits, steps)`, continuous and named-scale voices, versioned `.num` serialization with title, author, era, lineage, and editable prose credit, links, plotting, animation, singing, WAV and Standard MIDI File export of the sung melody on CLI, MCP, and the App F4 share, exact paused App reopen, the one-key share bundle, the local Gallery wall, and fork with recorded descent exist | Pattern Studio tracker, step grid, and piano roll, MusicXML, field melody, safe share preview for incoming links, and clean-install round trip |
...
Package label is **0.4.0-alpha.25** because Understanding Alpha is the active
...
4. Continue the Sensory Lift. GPU feasibility and the direct
   ...
   or unclassified timings pacing authority. The closed-set verifier is built;
   collect its six qualifying physical release-profile pacing receipts at 1080p
   and 1440p on Windows, macOS, and Linux before promotion, then re-baseline the
   visual and audio goldens once.
