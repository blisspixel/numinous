...
- **Built:** a plotted or sung Studio experiment names `next` as
  ...
  `overlay`. Pattern algebra and the in-app capability quest remain the
  next creator rungs.
...
| Real creative surface | Studio graphs, paired parametric paths, fields over the plane with three stored readings, and named sliders with declared ranges, continuous and named-scale voices, versioned `.num` serialization with title, author, era, lineage, and editable prose credit, links, plotting, animation, singing, WAV and Standard MIDI File export of the sung melody on CLI, MCP, and the App F4 share, exact paused App reopen, the one-key share bundle, the local Gallery wall, and fork with recorded descent exist | General multi-expression programs, MusicXML, field melody, safe share preview for incoming links, and clean-install round trip |
