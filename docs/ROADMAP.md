...
- **Done (catalog art-first and plate quality, cycles 161 to 163, PRs #63 to
  ...
  **0 dead-domain** rooms (base raster equals interacted), and **0 dead dials**
  ...
  a human gate.
- **Done (live motion pack, cycle 164):** classic curve and wave rooms stop
  being frozen plots. Ambient phase rolls generating circles (Cardioid,
  Cycloid, Astroid, Nephroid), walks pens (Limacon, Bifolium, Witch of Agnesi),
  unfurls the log spiral, oscillates standing waves, scrolls Fourier partials,
  and advances Poisson staircases. Docs codify the six-question plate filter
  and contact/MCP posture. Tests assert distinct ambient frames. Fun and
  motion before more densify grind.
