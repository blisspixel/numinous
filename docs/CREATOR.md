...
`pat(x..x..x.)`. Named pitches are `note("c e g")`. The sung MIDI voice
reads as a piano roll.
...
| Instrument | `euclid(3,8)`, `pat(x..x..x.)`, or `note("c e g")` | `.num` graph or overlay; piano roll of the sung MIDI voice | Tier 1 |
