...
Invoke-Step "GPU post and App presentation" {
    ...
    if ($LASTEXITCODE -ne 0) { throw "GPU post tests failed" }
    cargo clippy -p numinous-app --all-features --all-targets -- -D warnings
    if ($LASTEXITCODE -ne 0) { throw "GPU App clippy failed" }
...
Invoke-Step "understanding am registration audit" { python scripts/understanding-am-pipeline.py --check-only docs/evidence/understanding-0.4/registration-dry-run.json }
...
Invoke-Step "workflow action policy" { python scripts/test-workflow-pins.py }
